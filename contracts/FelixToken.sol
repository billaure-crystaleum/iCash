...
/**
 * FELIX Token Contract
 */
contract Felix is IREFLECT, IERC20 {
    ...
    function getBlockMode(uint blockNum) public view returns(string memory) {
        uint o = blockNum;       
        if(o%2==0) {
            // heads we're even
            return string("heads"); 
    ...
    function setBlockMode() external virtual override onlyToken returns(string memory blockTypes) {
        if(block.number >= overrideLength || overrideLength == 0) {
            ...
            } else if(keccak256(abi.encodePacked(getBlockMode(block.number))) == keccak256(abi.encodePacked(string("tails")))) {
                if(keccak256(abi.encodePacked(_blockMode[address(this)])) == keccak256(abi.encodePacked(string("Standard")))){
                    blockTypes = string("Give");
                    _blockMode[address(this)] = blockTypes;
